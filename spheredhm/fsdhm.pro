...
; p[3] : ap         radius of sphere
; p[4] : np         real part of sphere's refractive index
; p[5] : nm         real part of medium's refractive index
; p[6] : alpha_r    real part of alpha
; p[7] : alpha_i    imaginary part of alpha
;
; Optional:
; p[8] : kp         imaginary part of sphere's refractive index
; p[9] : km         imaginary part of medium's refractive index
...
np = p[4]
nm = p[5]
alpha = dcomplex(p[6], p[7])

if n_elements(p) ge 9 then $
   np = dcomplex(np, p[8])      ; sphere's complex refractive index
if n_elements(p) ge 10 then $ 
   nm = dcomplex(nm, p[9])      ; medium's complex refractive index
...
parinfo[6].limits[0] = 0.       ; cannot be negative
...
if nparams ge 9 then begin
   parinfo[8].fixed = keyword_set(fixnp)
   parinfo[8].limited[*] = 1
   parinfo[8].limits = [0.d, 5.d]
endif
if nparams ge 10 then begin
   parinfo[9].fixed = keyword_set(fixnm)
   parinfo[9].limited[*] = 1
   parinfo[9].limits = [0.d, 5.d]
endif
