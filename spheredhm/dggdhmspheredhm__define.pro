...
;    ALPHA:    ( GS) relative illumination.  Default = 1.0
; 
...
;;;;
;
; DGGdhmSphereDHM::ComputeCPU
;
; Compute hologram on CPU
;
pro DGGdhmSphereDHM::ComputeCPU
...
;;;;
;
; DGGdhmSphereDHM::Compute
;
; Compute hologram
;
pro DGGdhmSphereDHM::Compute
...
;           - \sin\phi \hat{\phi}
;
fac = self.alpha * exp(dcomplex(0, -k*self.rp[2]))
...
;;;;
;
; DGGdhmSphereDHM::SetProperty
;
; Set properties associated with the hologram
;
pro DGGdhmSphereDHM::SetProperty, xp = xp, $
                                  yp = yp, $
                                  zp = zp, $
                                  rp = rp, $
                                  ap = ap, $
...
;;;
; The following kludge is intended to move holograms
; sufficiently far off the computational grid to avoid
; divide-by-zero errors in computing geometry, but not
; so far as to affect fits.
; NOTE: This was dropped in favor of correcting divergences in
; Compute.
;res = (self.type eq 6) ? 1e-3 : 1d-7
...
if isa(alpha, /scalar, /number) then begin
...
;;;;
;
; DGGdhmSphereDHM::GetProperty
;
; Retrieve the value of properties associated with the hologram
;
pro DGGdhmSphereDHM::GetProperty, hologram = hologram, $
                                  field = field, $
                                  dim = dim, $
                                  ab = ab, $
                                  xp = xp, $
...
;;;;
;
; DGGdhmSphereDHM::GPUInit
;
; Initialize GPU and allocate GPU memory
;
function DGGdhmSphereDHM::GPUInit
...
;;;;
;
; DGGdhmSphereDHM::Init
;
; Initialize computational pipeline
;
function DGGdhmSphereDHM::Init, dim    = dim,    $ ; dimensions of hologram (R)
                                ...
                                alpha = alpha,   $ ; relative illumination
                                deinterlace = deinterlace, $
                                single = single
...
;;;;
;
; DGGdhmSphereDHM::Cleanup
;
; Free resources used by computational pipeline
;
pro DGGdhmSphereDHM::Cleanup
...
;;;;
;
; DGGdhmSphereDHM__define
;
; Define the object structure for a DGGdhmSphereDHM object
;
pro DGGdhmSphereDHM__define

COMPILE_OPT IDL2

struct = {DGGdhmSphereDHM,           $
          INHERITS     IDL_OBJECT,   $
          ...
          alpha:       0.D,          $ ; relative illumination
